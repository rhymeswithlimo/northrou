...
func (d *DB) UpsertShow(ctx context.Context, s *model.Show) (int64, error) {
	...
	err := d.WithTx(ctx, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(ctx, `
			INSERT INTO shows (tmdb_id, title, year, overview, original_lang, poster_path, backdrop_path, hero_backdrop_path, logo_path,
			...
			ON CONFLICT(tmdb_id) DO UPDATE SET
				...
				logo_path=excluded.logo_path, vote_average=excluded.vote_average,
				popularity=excluded.popularity, country=excluded.country,
				tagline=excluded.tagline, certification=excluded.certification`,
			s.TMDBID, s.Title, s.Year, s.Overview, s.OriginalLang, s.PosterPath, s.BackdropPath, s.HeroBackdropPath, s.LogoPath,
...
func (d *DB) GetShow(ctx context.Context, id int64) (*model.Show, error) {
	row := d.QueryRowContext(ctx, `
		SELECT id, tmdb_id, title, year, overview, original_lang, poster_path, backdrop_path, hero_backdrop_path, logo_path, added_at,
			vote_average, popularity, country, tagline, certification
		FROM shows WHERE id = ?`, id)
	var s model.Show
	err := row.Scan(&s.ID, &s.TMDBID, &s.Title, &s.Year, &s.Overview,
		&s.OriginalLang, &s.PosterPath, &s.BackdropPath, &s.HeroBackdropPath, &s.LogoPath, &s.AddedAt,
