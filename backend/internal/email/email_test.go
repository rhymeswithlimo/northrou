...
func TestDefaultedConfigUsesRelay(t *testing.T) {
	...
	// RelayURL and the relay is not disabled. Swap only the host for the probe
	// so the defaulted selection path is what is under test.
	cfg := config.Default()
	if cfg.Email.RelayURL == "" || cfg.Email.RelayDisabled {
...
func TestLogFallbackWhenRelayDisabled(t *testing.T) {
	srv, got := relayProbe(t)
	// relay_disabled => log fallback; the relay must not be called.
