...
func newScanCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runLibraryScan(args, asTV, false)
		},
	}
...
func newRescanCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "rescan [path...]",
		Short: "Re-scan and refetch metadata for every file, even unchanged ones",
		Long: "Like `scan`, but does not skip files that are already scanned and " +
			"unchanged: it re-probes and re-matches every file against TMDB, " +
			"refreshing stored metadata (artwork, logos, backdrops, credits). Use " +
			"it after upgrading to a version that adds new metadata your existing " +
			"library predates - a plain `scan` would report \"already scanned\" and " +
			"leave those titles untouched. It does a full TMDB pass, so it is slower " +
			"than a plain scan. Same arguments as `scan`; best run while the daemon " +
			"is stopped.",
		RunE: func(cmd *cobra.Command, args []string) error {
			return runLibraryScan(args, asTV, true)
		},
	}
	cmd.Flags().BoolVar(&asTV, "tv", false,
		"treat the given paths as TV episodes (default: detect movies vs episodes by filename)")
...
// runLibraryScan is the shared body of `scan` and `rescan`. When force is set it
// reprocesses every file (a full metadata refetch) instead of skipping unchanged
// ones.
func runLibraryScan(args []string, asTV, force bool) error {
	...
	if force {
		fmt.Println("Re-scanning (refetching metadata for every file)…")
	} else {
	...
	var scanErr error
	if force {
		scanErr = a.Scanner.Rescan(ctx, movieDirs, showDirs)
	} else {
