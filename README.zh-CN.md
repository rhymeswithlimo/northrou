...
`setup` 是交互式的，不需要任何参数。服务相关命令（`install`、`start`、`stop`、`restart`、`update`）会写入 root 权限的位置，因此在 Linux 上需要用 `sudo` 运行——命令会在需要提权时提示你。运行 `northrou <command> --help` 可以查看每个命令的完整参数列表。
